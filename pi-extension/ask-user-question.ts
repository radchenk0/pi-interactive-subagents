...
const AskUserQuestionParams = Type.Object({
	...
	options: Type.Optional(
		Type.Array(OptionSchema, {
			description:
				"Optional multiple-choice options. The number of options is NOT limited — provide as many (or as few) as logically needed for the question (2-3 for simple choices, 8+ for enumerations is fine). Omit or pass an empty array for free-form text input. Users will always be able to choose Other and type a custom answer when options are provided.",
		}),
	),
	multiSelect: Type.Optional(
		Type.Boolean({
			description:
				"Set to true when the question allows several correct answers at once (e.g. 'which features', 'select all that apply'). With multiSelect the user toggles any number of options before submitting.",
		}),
	),
});
...
export default function askUserQuestion(pi: ExtensionAPI) {
	// Sub-agent sessions (PI_SUBAGENT_ID set by the spawner) never register
	// the dialog — it must appear in the focused parent pane, not a child pane.
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"The number of options is not capped: offer as many options as the question logically requires (no artificial 2-4 limit); use as few as needed when fewer genuinely exist.",
			"Pass multiSelect: true whenever the question itself allows multiple correct answers (e.g. 'which of these…', 'select all that apply'); otherwise leave it unset for single-choice.",
